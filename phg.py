...
def render():
    for i, j in pixels:
        ...
        if min_t < 1e9:
            ...
            in_shadow = False
            shadow_ray_dir = L
            shadow_ray_orig = p + N * 1e-3  # 稍微偏移以避免自遮挡
            shadow_t_max = (light_pos - p).norm()
            
            # 检查阴影射线是否与场景中的物体相交
            # 检查球体
            t_shadow_sph, _ = intersect_sphere(shadow_ray_orig, shadow_ray_dir, ti.Vector([-1.2, -0.2, 0.0]), 1.2)
            if 0 < t_shadow_sph < shadow_t_max:
                in_shadow = True
            
            # 检查圆锥
            t_shadow_cone, _ = intersect_cone(shadow_ray_orig, shadow_ray_dir, ti.Vector([1.2, 1.2, 0.0]), -1.4, 1.2)
            if 0 < t_shadow_cone < shadow_t_max:
                in_shadow = True
